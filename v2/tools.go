...
func (t *Tools) UploadFiles(r *http.Request, uploadDir string, rename ...bool) ([]*UploadedFile, error) {
	...
	err := t.CreateDirIfNotExits(uploadDir)
	if err != nil {
...
//CreateDirIfNotExits creates a directory, and all necesary parents, if it does not exist
func (t *Tools) CreateDirIfNotExits(path string) error {
	const mode = 0755
...
func (t *Tools) Slugify(s string) (string, error) {
	...
	if len(slug) == 0 {
		return "", errors.New("after removing character, slug is zero length")
	}
...
//in the browser window by setting content dsposition. It also allows specification of the
//display name
...
//WriteJSON take a response status code and arbitrary data writes json to the client
...
//the final parameter, client, is opcional. If none is specified, we use the standard http.Client.
